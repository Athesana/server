...
<body>
	...
	<p>
		...
		지정한 URL로 현재 페이지의 요청과 응답에 대한 제어권을 영구적으로 넘긴다. <br>
	...
	<!-- 
		...
		즉, 한 번 요청에 대한 응답을 하면 더 이상 서버와 클라이언트와의 관계는 유지되지 않는다.
		로그인이나 장바구니 기능과 같이 클라이언트의 상태가 유지되어야 하는 프로그램에는 HTTP만 가지고는 구현하기가 어렵다.
		따라서 상태는 유지하지 않는 HTTP의 상태 특성 때문에 클라이언트과 서버의 상태(관계)를 유지하기 위한 기능이 세션이다. (similar. cookie)
		브라우저 단위로 세션이 생성된다. 브라우저를 닫고 다시 열면 세션을 다시 만들게 된다.
